export async function onRequest(context) {
  ...
  if (request.method === "GET") {
    ...
    try {
      ...
      const query = `
        ...
        p.is_solution
        FROM posts p WHERE p.topic_id = ? ORDER BY p.created_at ASC`;
  ...
  // === POST: Создать ответ + Уведомление ===
  if (request.method === "POST") {
    try {
      const data = await request.json();
      if (!data.topic_id || !data.content || !data.user_id) return new Response(JSON.stringify({ error: "Missing fields" }), { status: 400 });

      // 1. Сохраняем пост
      await db.prepare("INSERT INTO posts (topic_id, user_id, username, content) VALUES (?, ?, ?, ?)").bind(data.topic_id, data.user_id, data.username, data.content).run();

      // 2. Создаем уведомление автору темы
